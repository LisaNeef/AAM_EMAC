...
% and compute the covariance with the total.
% In this version, use a bootstrap method to compute the covariances,
...
%    total_name: name of the global variable to compute covariance for.
...
%    S_LO: lower bound of the covariances 98% confidence interval
%    S_HI: upper bound of the covariances 98% confidence interval
function covariance_map_bootstrap( runid, tscale, comp, var_name, total_name, fil_order, nboot, alpha   )
%------------------------------------------------------------------
...
% set flag for simple variable covariance
if strcmp(var_name,'EF'), simple_var = 0; else simple_var = 1; end

% set flag for covariance with aam (rather than observations)
...
% choose assumed decovariance time (in days) based on component and timescale
...
% initialize covariance / covariance matrix.
% for covariances between local EFs and a global number, SIG has 3 levels:
% mass, wind, and total.  For covariances with simple variables or
% observations, SIG is is simply latxlon.  Same for the others.
if simple_var
  SIG = zeros(nlat,nlon)+NaN;
  S_LO = zeros(nlat,nlon)+NaN;
  S_HI = zeros(nlat,nlon)+NaN;
else
  SIG = zeros(3,nlat,nlon)+NaN;
  S_LO = zeros(3,nlat,nlon)+NaN;
  S_HI = zeros(3,nlat,nlon)+NaN;
end
    ...
    if simple_var == 0
      ...
      for iday = 1:nd
        ...
        if isempty(targ) == 0
          ...
          % if computing covariance with total, integrate here, and also
      ...
      % If computing variable covariances, simply sort u, v, and ps into x.
      ...
      % that's what we are computing the covariance with.
...
for ii = 1:nlevels
    disp('Computing covariances for component')
    disp(ii)
  for ilat = 1:nlat
      disp(ilat)
    for ilon = 1:nlon
      ...
      % CHANGED 25 Oct 2011 (during paper revisions 2nd round):
      % filter the local timeseries too; this should make the covariances
      % more pronounced.
      ...
      if corr_with_ERP
          % covariance with observations
          y = F(g)';
      else
          % covariance with global AEF - respective wind or mass term
          % only.
          y = F(ii,g)';
      end
      ...
      B = zeros(nboot,1);
      for iboot = 1:nboot
          dum = cov(x(bsdata(:,iboot)),y(bsdata(:,iboot)));
          B(iboot) = dum(1,2);
      end
      Bsort = sort(B);
      
      
      cov_ci = [Bsort(round((alpha/2)*nboot),:); Bsort(round((1-alpha/2)*nboot),:)] ; %Report confidence intevals
      cov_out = mean(Bsort);
      %corr_std = std(B(:,2));
            
      if simple_var
        SIG(ilat,ilon) = cov_out;
        S_LO(ilat,ilon) = cov_ci(1);
        S_HI(ilat,ilon) = cov_ci(2);
      else
        SIG(ii,ilat,ilon) = cov_out;
        S_LO(ii,ilat,ilon) = cov_ci(1);
        S_HI(ii,ilat,ilon) = cov_ci(2);        
      end
...
if corr_with_ERP
    savename1 = ['cov_',var_name,'_obsX',num2str(comp),'_',runid,'_',tscalename,'_filorder',num2str(fil_order)];
else
   savename1 = ['cov_',var_name,'_X',num2str(comp),'_',runid,'_',tscalename,'_filorder',num2str(fil_order)]; 
end
savename = [savename1,num2str(date_start(1)),'-',num2str(date_stop(1)),'_bootstrap',num2str(nboot),'.mat'];
save(savename, 'SIG', 'S_LO', 'S_HI', 'lat', 'lon','nboot','alpha')
